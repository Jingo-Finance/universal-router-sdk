...
import { Trade as V1Trade, Pair } from '@jingofi/v1-sdk'
import { Trade as V3Trade, Pool, encodeRouteToPath } from '@jingofi/v3-sdk'
import {
...
} from '@jingofi/router-sdk'
import { Permit2Permit } from '../../utils/inputTokens'
import { Currency, TradeType, CurrencyAmount, Percent } from '@jingofi/sdk-core'
  ...
  encode(planner: RoutePlanner, _config: TradeConfig): void {
    ...
    for (const swap of this.trade.swaps) {
      switch (swap.route.protocol) {
        case Protocol.V1:
          addV1Swap(planner, swap, this.trade.tradeType, this.options, payerIsUser, routerMustCustody)
          break
        case Protocol.V3:
...
function addMixedSwap<TInput extends Currency, TOutput extends Currency>(
  ...
  if (route.pools.length === 1) {
    if (route.pools[0] instanceof Pool) {
      ...
      return addV1Swap(planner, swap, tradeType, options, payerIsUser, routerMustCustody)
    } else {
